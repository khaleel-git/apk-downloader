...
$headers = get_headers($apk_pure, 1);

if ($headers) {
    // Check the status code and print the corresponding message
    if ($headers[1] === 'HTTP/1.1 301 Moved Permanently') {
       // echo "Received a 301 Moved Permanently header.";
    } elseif ($headers[1] === 'HTTP/1.1 200 OK') {
        //echo "Received a 201 OK header.";
    } else {
        //echo "Received a different header.<br>";
      	//echo $headers[1];
    }
    ...
    $fileName = 'apkpure-headers/' . $appId . '.txt';
    file_put_contents($fileName, $headerString);

    //echo 'Headers written to headers.txt file.';
} else {
    echo "Unable to retrieve headers for the URL.";
}

if ($headers[1] === 'HTTP/1.1 301 Moved Permanently'){
    $apk_pure = "https://d.apkpure.com/b/XAPK/" . $appId . "?version=latest";
}



$file_pointer_json = "https://apkfuel.com/apk-downloader/user_content/play_json/$appId.json";
...
if(isset($_GET['src'])){
	...
	if($source == 'apr'){
		...
		$temp_title[1] = (float)$temp_title[1];
		// echo "After splitting: $temp_title[0] <br>";
		// echo "<br>The numeric part is: $temp_title[1] <br>";
	
	if ( is_numeric($temp_title[1]) ) {
    	// echo "<br> Number is: $temp_title[1] <br>";
    	$title = $temp_title[0];
    	// echo "<br> Number found, now the title without number is: $title <br>";
    	$title = preg_replace('/[^a-z0-9]+/', '-', strtolower($title));
		$title = str_replace("-8211-", "-", $title);
	} 
    else {
...
.w-button-green-out {align-items: center;background: transparent;border: 0;border-radius: 5px;box-shadow: none;background-color: #F15A24;color: white;cursor: pointer;display: inline-flex;font-size: 1.0em;font-weight: 500;justify-content: center;letter-spacing: 1px;outline: 0;overflow: hidden;padding: 9px 9px;position: relative;text-decoration: none;transition: background-color .2s, box-shadow .2s;-moz-osx-font-smoothing: grayscale;-webkit-font-smoothing: antialiased;text-rendering: optimizeLegibility;box-shadow: 0 2px 4px -1px rgba(0, 0, 0, .2), 0 4px 5px 0 rgba(0, 0, 0, .14), 0 1px 10px 0 rgba(0, 0, 0, .12);outline: none;text-decoration: none;background:linear-gradient(to right,#F15A24 50%,#689f38 50%);background-size:200% 100%;background-position:left bottom;margin-left:10px;transition:all 2s ease }.w-button-green-out:hover {box-shadow: 0 4px 8px -2px rgba(0, 0, 0, .2), 0 8px 10px 0 rgba(0, 0, 0, .14), 0 2px 20px 0 rgba(0, 0, 0, .12);outline: none;text-decoration: none; background-position: right bottom;}.w-button-green-out-disable {cursor: not-allowed;pointer-events: none;color: #9E9E9E;background-color: #e6e6e6;}
	...
	<link rel="stylesheet" href="https://apkfuel.com/apk-downloader/assets/apk.php/yellowcircle.css">
	<link rel="stylesheet" href="https://apkfuel.com/apk-downloader/assets/apk.php/purplecircle.css">
...
<div style="margin-bottom: 5px !important; margin-top: 1px; display: flex;justify-content: center;align-items: center;">
		<img  src= "<?php echo $json_obj['icon'] ?>" alt="App icon" width="110"/></br>  
		</div>
...
<script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-1443182149991411"
     crossorigin="anonymous"></script>
<!-- travel accident lawyer -->
<ins class="adsbygoogle"
     style="display:block"
     data-ad-client="ca-pub-1443182149991411"
     data-ad-slot="1330366066"
     data-ad-format="auto"
...
(function download() {	
    ...
    // Perform the necessary actions to initiate the download
    document.getElementById('dl').click();
	
    // Show the download button again (if needed)
